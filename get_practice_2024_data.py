...
def get_session_keys(grand_prix, year=2024):
    ...
    if response.status_code == 200:
        data = response.json()
        # Filter sessions for Practice 1, 2, and 3
        practice_sessions = [session for session in data if session['session_name'] in [
...
def collect_data_for_practice():
    ...
    for gp in grand_prix:
        ...
        for session_name, (session_key, circuit_short_name) in session_keys.items():
            ...
            if session_name == 'Practice 3' and not mclaren_drivers:
                mclaren_drivers = [4, 81]
                print(
                    f"Falling back to known McLaren drivers for Practice 3: {mclaren_drivers}")
    ...
    with pd.ExcelWriter('mclaren_practice_2024_data.xlsx') as writer:
        for gp, data in all_gp_data.items():
            data.to_excel(writer, sheet_name=gp, index=False)

    print("Data saved to mclaren_practice_2024_data.xlsx")
    print()


collect_data_for_practice()
